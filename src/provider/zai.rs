...
impl ZAISwitcher {
    ...
    pub fn switch_to_zai(&self) -> Result<()> {
        println!("{}", "🔄 Switching to Z.AI API...".green());
        ...
        if ProviderDetector::is_zai_config(&config) {
            println!("{}", "⚠️  Already using Z.AI configuration".yellow());
        ...
        // Get Z.AI API token
        let token = self.token_manager.prompt_for_token(&self.config_manager)
            .context("Failed to get Z.AI API token")?;

        // Validate token format
        ProviderDetector::validate_token_for_provider(&token, &Provider::ZAI);

        // Create new config for Z.AI
        let new_config = self.create_zai_config(&token);

        self.config_manager.save_current_config(&new_config)
            .context("Failed to save Z.AI configuration")?;

        println!("{}", "✅ Z.AI configuration applied successfully".green());
        println!();
        println!("{}", "💡 To switch back to Anthropic: claude-switch --anthropic".cyan());
        Ok(())
    }

    fn backup_anthropic_config_if_needed(&self, config: &Config) -> Result<()> {
        // Check if valid Anthropic backup already exists
        let (has_backup, existing_backup) = self.config_manager.has_valid_anthropic_backup()
            .context("Failed to check existing backup")?;

        if has_backup && existing_backup.is_some() {
            // Backup already exists - don't overwrite
            println!("{}", "💾 Existing Anthropic backup found (preserving configuration)".cyan());
            ...
            self.config_manager.create_backup_with_metadata(config, &Provider::Anthropic)
    ...
    fn handle_unknown_provider(&self) -> Result<()> {
        ...
        if has_backup {
            ...
            println!("{}", "   You may need to re-login when switching back".yellow());
        }
        Ok(())
    }

    fn handle_custom_provider(&self) {
        println!("{}", "⚠️  Current config is custom provider - not backing up".yellow());
        println!("{}", "   Anthropic backup will be preserved if it exists".yellow());
    }

    fn create_zai_config(&self, token: &str) -> Config {
        ...
        env.insert("ANTHROPIC_DEFAULT_OPUS_MODEL".to_string(), "GLM-4.6".to_string());
        env.insert("ANTHROPIC_DEFAULT_SONNET_MODEL".to_string(), "GLM-4.6".to_string());
        env.insert("ANTHROPIC_DEFAULT_HAIKU_MODEL".to_string(), "GLM-4.5-Air".to_string());
